...
class TileMap:
    def __init__(self, tile_size, view_size):
        ...
        self.all_layers = []
        self.dropthroughs = []
        self.physical_check = lambda x : True
    ...
    def split_map(self):
        ...
        for pos in list(self.tile_map):
            for layer in list(self.tile_map[pos]):
                if not self.physical_check(self.tile_map[pos][layer]['type'][0]):
                    if pos not in self.background_tile_map:
                        self.background_tile_map[pos] = {}
                    self.background_tile_map[pos][layer] = self.tile_map[pos][layer]
                    del self.tile_map[pos][layer]
                    if len(self.tile_map[pos]) == 0:
                        del self.tile_map[pos]

    def load_grass(self, gm):
        grass_filter = lambda x: x['type'][0] == 'grass'
        for tile in self.tile_filter(grass_filter):
            gm.place_tile(tile[0], random.randint(8, 14), [0, 1, 1, 2, 2, 3, 3, 4, 4, 5, 6])
    ...
    def get_nearby_rects(self, pos):
        ...
        for p in SURROUND_POS:
            ...
            if check_loc in self.tile_map:
                ...
                if not any([self.physical_check(tileset[0]) for tileset in rects[-1][0]]):
                    rects.pop(-1)
        return rects
    ...
    def tile_collide(self, pos):
        ...
        if tile_pos in self.tile_map:
            for layer in self.tile_map[tile_pos]:
                if self.tile_map[tile_pos][layer]['type'][0] in ['main_tileset', 'grass_tileset', 'wood_tileset']:
                    return True
        return False
    ...
    def get_visible(self, pos, size_override=None):
        ...
        for y in range(math.ceil(size[1] / self.tile_size[1]) + 1):
            for x in range(math.ceil(size[0] / self.tile_size[0]) + 1):
                ...
                # if tile_pos in self.background_tile_map:
                #     for tile in self.background_tile_map[tile_pos]:
                #         layers[tile].append(self.background_tile_map[tile_pos][tile]['raw'] + ([self.background_tile_map[tile_pos][tile]['entity_id']] if 'entity_id' in self.background_tile_map[tile_pos][tile] else []))
