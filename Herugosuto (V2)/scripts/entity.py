...
class Entity:
    def __init__(self, game, pos, size, type):
        # self.assets = assets  # asset manager going to be from game data
        self.game = game
        ...
        if self.type + '_idle' in self.game.animations.animations:
    ...
    def center(self):
        ...
        elif self.active_animation:
            return [self.pos[0] + self.active_animation.data.config['offset'][0] + self.size[0] // 2, self.pos[1] + self.active_animation.data.config['offset'][1] + self.size[1] // 2]
        else:
            return [self.pos[0] + self.size[0] // 2, self.pos[1] + self.size[1] // 2]

    def set_action(self, action_id, force=False):
        if force:
            self.active_animation = self.game.animations.new(self.type + '_' + action_id)
    ...
    def move(self, motion, tiles, tile_map=None):
        self.pos[0] += motion[0]
        if tile_map:    
            tiles = tile_map.get_nearby_rects(self.pos)
